    ...
    async def connect(self) -> None:
        ...
        try:
            if "socket" in self.config:
                ...
                raise ValueError(
                    "Invalid Redis config: must specify 'socket' or 'host' and 'port'"
                )
        ...
        async with self.redis_pool.acquire() as redis:
            async with redis.pipeline() as pipe:
                for rd in rdata:
                    ...
                    if current_lastseen is None or int(record.time_last) > int(
                        current_lastseen
                    ):
                ...
                if record.sensor_id:
                    pipe.hincrby(
                        f"sensor:{record.sensor_id}", "count", record.count or 1
                    )
        ...
        async with self.redis_pool.acquire() as redis:
            try:
                ...
                if query_key:
                    ...
                    rdata_set = set()
                    while True:
                        cursor_scan, keys = await redis.scan(
                            cursor_scan, match=f"r:{rrname}:*", count=100
                        )
                        for key in keys:
                ...
                rrtypes = (
                    [rrtype]
                    if rrtype
                    else [k.split(":")[-1] for k in keys] if not query_key else []
                )
        ...
        async with self.redis_pool.acquire() as redis:
            ...
            while True:
                cursor, keys = await redis.scan(
                    cursor, match=f"pdns:{q}:*", count=chunk_size
                )
                for key in keys:
    ...
    async def get_associated_records(self, q: str) -> List[str]:
        ...
        async with self.redis_pool.acquire() as redis:
            try:
                ...
                logger.error(
                    {
                        "event": "redis_get_associated_records_failed",
                        "query": q,
                        "error": str(e),
                    }
                )
