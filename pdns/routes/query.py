...
async def query(
    request: Request,
    q: str,
    cursor: Optional[str] = Query(default=None, description="Cursor for pagination, use to fetch next page if total > limit"),
    limit: int = Query(default=200, ge=1, le=1000, description="Max records per page or total without cursor"),
    rrtype: Optional[SupportedRRType] = Query(default=None, description="Filter by supported RR type (e.g., A, AAAA)"),
    ...
    if format == ResponseFormat.ndjson:
        response_content = "\n".join(record.to_ndjson(time_format) for record in dns_records)
